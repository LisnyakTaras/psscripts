# Version 2.6
# Данный скрипт отправляет сообщения всем (или выбранным пользователям удаленного рабочего стола)
...
# Дописываем подписи по вкусу/желанию
$Sign          = @("С уважением, администраторы 1С", "С уважением, системные администраторы", "Твой Сис-Админ!!!")

# Повелось, что все функции описываются до их использования.
# Функция отправки сообщения. Принимаем параметры, проверяем, если чекбокс включен (а он включен по умолчанию), устанавливаем параметры отправки
# только на консоль локалхоста, иначе проходим по списку серверов, внутри каждого сервера инициируем цикл по списку пользователей с предварительно
# отрезанными пробелами до и после текста(помните формат: "сервер1, сервер2, сервер3" для красивой читаемости текста), вызываем сам msg для отправки
# сообщения и после чего вызываем диалогов окно с подтверждением отправки и закрытием программы.
...
$SendButton.add_click({ $User  = $UserTextBox.Text.Split(","); $Server = $ServerTextBox.Text.Split(",");$Message = $MessageTextBox.Text; $SignX = $SignTextBox.Text; SendMessage $Server $User $Message $SignX} )
